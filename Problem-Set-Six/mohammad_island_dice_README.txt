Author: Nayalash Mohammad

Class: ICS3U1 - Period 2 - Berardi, A

Date: October 31 2019

Version: 1.0.0

Unit 2: Problem Set Six, Question 1

Programming Language: Python 3.7.6

Description: A program that follows the island dice game algorithm.

Running File Name: mohammad_island_dice.py

Program Assumptions: In order to run this program, you need a computer running either MacOS, Windows or Linux, with Python 3 or greater, having the Python Environmental Variable set to your PATH

Commands:

    • Run => run
    • Quit => quit
    • help => help

Features:

    • A always on menu, where the menu always shows up after each computation is completed.
    • Simple and easy to use console interface, with program usage help.

Restrictions:

    • No Exception Handling if user gives an input outside range or invalid DATA TYPE input occurs.

Known Errors:

    • The shells sometimes do not update, which requires a restart of the program

Implementation Details:

    • Island Dice Game Algorithm
    • To have the continuously running menu, I applied the methodology of recursive functions.

Additional Files:
    • mohammad_island_dice.py
    • mohammad_island_dice_README.txt
